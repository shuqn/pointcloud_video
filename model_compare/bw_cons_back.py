...
RESEVOIR = 1  # BB
CUSHION = 3  # BB
LOG_FILE = current_directory+'/results/log_sim_bw_cons_back'
RESULT_FILE=current_directory+'/results/result_sim_bw_cons_back'
...
def test(cooked_time,cooked_bw,video_size,fov,dis,time=None):
    ...
    log_file.write(str(round(time_stamp,3)) + '\t' + str(player_fov_count+player_fractional_time/VIDEO_GOF_LEN)+ '\t' + 
                    ...
                    str(selected_tile) + '\t'+ 
                    str(bit_rate)+'\n')
    gof_choosen=[0]*(FRAME//F_IN_GOF)
    while True:      
        log_file.flush()
        back=0
        if fov_count-player_fov_count-player_fractional_time/VIDEO_GOF_LEN>0 and player_fov_count+player_fractional_time>0 : #back
            for predict_window in range(player_fov_count+math.ceil(player_fractional_time/VIDEO_GOF_LEN+0.1),fov_count):
                ...
                if sum(selected_tile)>0:
                    ...
                    pre_len=(predict_window)*F_IN_GOF-player_fov_count*F_IN_GOF
                    recall_ratio=max(-0.004375*pre_len+1,0)
                    # if pre_len<=40:
                    #     recall_ratio=1
                    recall_ratio-=gof_choosen[predict_window]
                    # gof_choosen[predict_window]+=recall_ratio
                    bit_rate=[0]*TILE_IN_F
                    bw_constrain= net_env.predict_bw()*recall_ratio
                    bit_constrain=bw_constrain*VIDEO_GOF_LEN+0.1
                    min_size=0
                    for i in range(F_IN_GOF):
                        for t in range(TILE_IN_F):
                            if selected_tile[t]:
                                min_size+=video_size[predict_window*F_IN_GOF+i][t][0]
                            pass
                            # min_size+=t[0]
                    if min_size>=bit_constrain:
                        pass
                    else:
                        cur_dis=dis[predict_window*F_IN_GOF]
                        ...
                        for cur_l in range(QUALITY_LEVELS-1, 0, -1):
                            ...
                            for tile in index_dis:
                                ...
                                for f in range(F_IN_GOF):
                                    add_bit+=video_size[predict_window*F_IN_GOF+f][tile][cur_l]
                                    sub_bit+=video_size[predict_window*F_IN_GOF+f][tile][0]
                                if selected_tile[tile] and  min_size+add_bit-sub_bit<=bit_constrain and bit_rate[tile]==0:
                                    bit_rate[tile]=cur_l
                                    min_size=min_size+add_bit-sub_bit     
                    gof_choosen[predict_window]+=recall_ratio*min_size/bit_constrain
                    
                    delay,buffer=net_env.get_video_gof_back(predict_window,selected_tile,bit_rate)
                    time_stamp+=delay
        ...
        if back==0 and fov_count<FRAME//F_IN_GOF:#new 
            ...
            selected_tile=[ math.ceil(sum(col) / len(col)) for col in zip(*selected_tile)]
            pre_len=(fov_count)*F_IN_GOF-player_fov_count*F_IN_GOF
            recall_ratio=max(-0.004375*pre_len+1,0)
            # if pre_len<=40:
            #     recall_ratio=1
            # gof_choosen[fov_count]+=recall_ratio
            bit_rate=[0]*TILE_IN_F
            real_bw=net_env.predict_bw()
            bw_constrain= real_bw*recall_ratio
            real_bit=real_bw*VIDEO_GOF_LEN
            bit_constrain=bw_constrain*VIDEO_GOF_LEN+0.1
            min_size=0
            for i in range(F_IN_GOF):
                for t in range(TILE_IN_F):
                ...
                for cur_l in range(QUALITY_LEVELS-1, 0, -1):
                    ...
                    for tile in index_dis:
                        ...
                        if selected_tile[tile] and  min_size+add_bit-sub_bit<=bit_constrain and bit_rate[tile]==0:
                            bit_rate[tile]=cur_l
                            min_size=min_size+add_bit-sub_bit     
            gof_choosen[fov_count]+=min_size/real_bit
            delay,buffer=net_env.get_video_gof_new(selected_tile,bit_rate)
    ...
    open(result_path+'gof_choo', 'a').write(str(time)+str(gof_choosen)+'\n') 
